...
def fix_marks(schoolkid):
    ...
    try:
        ...
        marks = Mark.objects.filter(schoolkid=user, points__in=[2, 3])
        for mark in marks:
            mark.points = 5
            mark.save()
    except Schoolkid.DoesNotExist:
        print(f'{schoolkid} не найден в базе данных. Проверьте правильность'
              f'ввода')
...
def create_commendation(schoolkid, lesson_title):
    ...
    try:
        user = Schoolkid.objects.get(full_name__contains=schoolkid)
        lesson = Lesson.objects.filter(group_letter=user.group_letter,
                                       year_of_study=user.year_of_study,
                                       subject__title=lesson_title)\
            .order_by('-date').first()
        Commendation.objects.create(text=get_commedation(), schoolkid=user,
                                    created=lesson.date,
        ...
        print(f'{lesson_title} не найден в базе данных. Проверьте '
              f'правильность ввода')
    except Schoolkid.MultipleObjectsReturned:
        print(f'Найдено более 1 записи - {schoolkid}. Укажите полное ФИО '
              f'ученика')
